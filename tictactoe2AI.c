...
printf("COMPUTER ONE: X  COMPUTER TWO: O\n");
...
void play_game (char board[9]) {
    ...
    while (player_turn <= 10 && (check_game_victory(board) == -1)) {
        ...
        if (player_turn % 2 == 0) {
            ...
            printf("COMPUTER ONE'S TURN, CHOOSE A LETTER TO PLACE X:\n");

            srand(time(0));
            char computer_character1 = rand() % (105-97) + 97 ;  
                      
            
            //Scenario: wrong leter entered or spot already taken           
            //While loop allows user to try again until correct letter          
            while (!((computer_character1 >= 'a' && computer_character1 <= 'i')
            && (computer_character1 == board[computer_character1 - 97]))) {
                
                if (!(computer_character1 >= 'a' && computer_character1 <= 'i')) {
                    printf("WRONG LETTER OR INVALID INPUT! PLEASE TRY AGAIN:\n");
                }
                
                else if (computer_character1 != board[computer_character1 - 97]) {
                    printf("THIS SPOT HAS ALREADY BEEN TAKEN! PLEASE TRY AGAIN:\n");
                }
                
                computer_character1 = rand() % (105-97) + 97 ;  
            }

                     
            //Scenario: correct letter enetered:
            if (computer_character1 >= 'a' 
            && computer_character1 <= 'i') {
                              
                board[computer_character1 - 97] = 'X'; 
                print_board(board);              
            }
        }
        ...
        //For player two: 
            ...
            printf("COMPUTER TWO's TURN: \n");
            
            srand(time(0));
            char computer_character2 = rand() % (105-97) + 97 ;  
    
            //Scenario: wrong leter entered or spot already taken
            //Can try again until correct letter                       
            while ( !((computer_character2 >= 'a' && computer_character2 <= 'i')
            && (computer_character2 == board[computer_character2 - 97])) ) {
                        
                if (!(computer_character2 >= 'a' && computer_character2 <= 'i')) {
                    printf("WRONG LETTER OR INVALID INPUT! PLEASE TRY AGAIN:\n");
                }
                
                else if (computer_character2 != board[computer_character2 - 97]) {
                    printf("THIS SPOT HAS ALREADY BEEN TAKEN! PLEASE TRY AGAIN:\n");
                }

                computer_character2 = rand() % (105-97) + 97 ;  
            }
            
            //Scenario: correct letter enetered:
            if (computer_character2 >= 'a' 
            && computer_character2 <= 'i') {
                              
                board[computer_character2 - 97] = 'O';                        
                print_board(board);     
...
int check_game_victory (char board[9]) {
    ...
    if ( (board[0] == board[3] && board[3] == board[6]) && (board[0] == 'X')) {
        printf("GAME OVER! COMPUTER ONE WINS!!\n");
        return 1;
    }
    
    else if ( (board[0] == board[3] && board[3] == board[6]) && (board[0] == 'O')) {
        printf("GAME OVER! COMPUTER TWO WINS!!\n");
        return 1;
    }
